...
public:
	...
	int size() {return size(begin(),0) ; /*etapa 2: implemente esta funcao sem usar o membro de dados dataSize*/ } // na STL List, a funcao size() calcula o tamanho da lista dinamicamente (exercicio: qual a ordem de complexidade?)
	int size(iterator t,int cont);
private:
...
public:
	...
	MyList2Iterator(Node<T> *p,MyList2<T> *l){
		ptr=p;
		lista=l;
	}
...
MyList2Iterator<T> MyList2Iterator<T>::operator--() {
	if(ptr==NULL){
		MyList2Iterator t=lista->begin();
		MyList2Iterator t1=lista->begin();
		while (t!=NULL){
		t1=t;
		t++;
		}
...
template<class T>
int MyList2<T>:: size(iterator t,int cont){
	if(t==end()){return cont;}
	return size(++t,cont+1);
}
