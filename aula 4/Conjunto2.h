...
Conjunto<T> Conjunto<T>::operator+(const Conjunto<T> &other){
    Conjunto<T>B(tam_array+other.tam_array);
	for(int i=0;i<numelementos();i++){
        B.elementos[i]=elementos[i];
    }
    B.num_elementos=num_elementos;
...
Conjunto<T> Conjunto<T>::operator*(const Conjunto<T> &other){
	int menor;
	if(tam_array<other.tam_array){
		menor=tam_array;
	}
	else{
		menor=other.tam_array;
	}
	Conjunto<T>B(menor);
	for (int i = 0; i < numelementos(); i++){
    	if (other.pertence(elementos[i])){
            B.elementos[B.numelementos()]=elementos[i];
            B.num_elementos++;
        }    
    }
    return B;
