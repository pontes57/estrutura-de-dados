...
template<class T>
void MyList<T>::apaga(iterator t){
	if(t==NULL){return;}
	apaga(t->next);
	delete t;
}
...
int MyList<T>::eraseMatchingElements(const T&elem){
	...
	iterator anterior=dataFirst;
	iterator temp=dataFirst->next;
	iterator ajuda;
	int contador=0;
	while (temp!=NULL){
		if(temp->data==elem){
...
template<class T>
Node<T>* MyList<T>::inverte (Node<T>* i){
    if ((i->next)->next == NULL){
		(i->next)->next=i;
		dataFirst=dataLast;
        return i;
